...
const addMethods = function(){
  ...
  // Now create our handle
  const makeTransformable = function(selector, callback)
  {
    console.error("Adding handles to "+selector, $(selector));

    $(selector).each( function(i, element) {
      console.error("Adding handles to ", element );

    });

    return $(selector).each( function(i, element) {

      let p, position, $element = $(element);

      // set initial transform
      $element.css('transform', '');
      console.log("Adding handles to transformer");

      let controlPoints = (function() {
  ...
  return {
    ...
    makeTransformable:makeTransformable
  }
};
...
// Scripts loaded
let loadCount  = 0;
const onScriptsLoaded = function()
{
  console.log("Script loaded");
  // when loadCount = 2
  if ( ++loadCount >= 2 )
  {
    let methods = addMethods();
    methods.makeTransformable( "#tweets" );
  }
};

jQuery.getScript('//ajax.googleapis.com/ajax/libs/jqueryui/1.10.3/jquery-ui.min.js', onScriptsLoaded);
jQuery.getScript('//cdnjs.cloudflare.com/ajax/libs/numeric/1.2.6/numeric.min.js', onScriptsLoaded);
